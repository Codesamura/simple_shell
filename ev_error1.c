...
void prnt_err(info_t *info, char *estr)
{
	_eputs(info->fname);
	_eputs(": ");
	pr_dec(info->line_count, STDERR_FILENO);
	_eputs(": ");
	_eputs(info->argv[0]);
	_eputs(": ");
	_eputs(estr);
...
/**
 * pr_dec - function prints a decimal (integer) number (base 10)
 * @input: the input
...
/**
 * cvnt - converter function, a clone of itoa
 * @num: number
 * @base: base
